...
export default function QnaList({ postId, postOwnerId }: { postId: number; postOwnerId: number }) {
  const { userInfo } = useUser(); // 로그인한 유저 정보
  const isOwner = userInfo?.user_id === postOwnerId; // 로그인한 유저가 게시글 작성자와 같은지 확인하고 답변 권한 부여
    ...
    <div className="space-y-4">
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="mt-6 p-4 border text-sm text-gray-600 bg-gray-50 rounded-md">
          ...
          <div>
            🙋 <strong>로그인한 유저 ID</strong>: {userInfo?.user_id ?? "불러오지 못함"}
          </div>
          <div>
